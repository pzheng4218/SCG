...
for k in range(1, outer_epoch+1):
    ...
    w_ref = D3[-1]
    G = (w_ref - prox_l1_norm(w_ref - 0.5 * sarah.normalized_sigmoid_grad(w_ref), lam1, 0.5)) / 0.5
    SS3.append(sarah.normalized_sigmoid_function(w_ref) - star-0.001)
    S3.append(np.linalg.norm(G, ord=2) ** 2)
    D3 = []
...
for k in range(1, outer_epoch+1):
    ...
    w_ref = D4[-1]
    G = (w_ref - prox_l1_norm(w_ref - 0.5 * sarah.normalized_sigmoid_grad(w_ref), lam1, 0.5)) / 0.5
    SS4.append(sarah.normalized_sigmoid_function(w_ref) - star-0.001)
    S4.append(np.linalg.norm(G, ord=2) ** 2)
    D4 = []
...
# plt.savefig('p2_gisette_l.png', dpi=600)
plt.show()
...
font1 = {'size': 7}
plt.legend(handles=[line1, line2, line3, line4], prop=font1)
# plt.savefig('p2_gisette_G.png', dpi=600)
plt.show()