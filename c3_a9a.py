...
G = (w_ref - prox_l1_norm(w_ref - 0.5 * sarah.difference_logistic_grad(w_ref), lam1, 0.5)) / 0.5
D1.append(w_ref)
SS1.append(sarah.difference_logistic_function(w_ref) + lam1 * np.linalg.norm(w_ref, 1) - star)
S1.append(np.linalg.norm(G, ord=2) ** 2)

h = sarah.difference_logistic_grad(w_ref)
for k in range(1, outer_epoch+1):
    omega = w_ref
    omega_old = omega
    v = sarah.difference_logistic_grad(omega)
    v_old = v
    d = -h
    eta = 0.1
    line_search = 0
    ...
    while line_search == 0:
        ...
        v_new = sarah.difference_logistic_grad(omega_l, batch_b) - sarah.difference_logistic_grad(omega_old_l, batch_b) + v
        x = abs(v_new.dot(d))
        y = -c_2 * v.dot(d)
    ...
    for j in range(1, inner_loop+1):
        v = sarah.difference_logistic_grad(omega, batch_b) - sarah.difference_logistic_grad(omega_old, batch_b) + v_old
        # beta1-FR
...
G = (w_ref - prox_l1_norm(w_ref - 0.5 * sarah.difference_logistic_grad(w_ref), lam1, 0.5)) / 0.5
D2.append(w_ref)
SS2.append(sarah.difference_logistic_function(w_ref) + lam1 * np.linalg.norm(w_ref, 1) - star)
...
w_ref = np.array([0.]*sarah.dim)
G = (w_ref - prox_l1_norm(w_ref - 0.5 * sarah.difference_logistic_grad(w_ref), lam1, 0.5)) / 0.55
D3.append(w_ref)
SS3.append(sarah.difference_logistic_function(w_ref) + lam1 * np.linalg.norm(w_ref, 1) - star)
...
G = (w_ref - prox_l1_norm(w_ref - 0.5 * sarah.difference_logistic_grad(w_ref), lam1, 0.5)) / 0.5
D4.append(w_ref)
SS4.append(sarah.difference_logistic_function(w_ref) - star)
...
# omega = w0
# G = (omega - prox_l1_norm(omega - 0.5 * sarah.difference_logistic_grad(omega), lam1, 0.5)) / 0.5
# SS4.append(sarah.difference_logistic_function(omega) + lam1 * np.linalg.norm(omega, 1) - star)
# S4.append(np.linalg.norm(G, ord=2) ** 2)
# pass4 = [0]
#
# v = 0
...
G = (w_ref - prox_l1_norm(w_ref - 0.5 * sarah.difference_logistic_grad(w_ref), lam1, 0.5)) / 0.5
D5.append(w_ref)
SS5.append(sarah.difference_logistic_function(w_ref) + lam1 * np.linalg.norm(w_ref, 1) - star)
...
for k in range(1, outer_epoch+1):
    ...
    omega_old = omega
    batch_B = random.sample(list(range(sarah.num)), size_B)
    phi = sarah.difference_logistic_grad(w_ref, batch_B)
    v = phi
    omega_h = omega_old - eta * v
    omega = prox_l1_norm(omega_h, lam1, eta)
    D5.append(omega)
    for j in range(1, inner_loop+1):
        batch_b = random.sample(list(range(sarah.num)), size_b)
        v = sarah.difference_logistic_grad(omega, batch_b) - sarah.difference_logistic_grad(w_ref, batch_b) + phi
        omega_old = omega
        omega_h = omega_old - eta * v
        omega = prox_l1_norm(omega_h, lam1, eta)
        D5.append(omega)
...
G = (w_ref - prox_l1_norm(w_ref - 0.5 * sarah.difference_logistic_grad(w_ref), lam1, 0.5)) / 0.5
D6.append(w_ref)
SS6.append(sarah.difference_logistic_function(w_ref) + lam1 * np.linalg.norm(w_ref, 1) - star)
...
for k in range(1, outer_epoch+1):
    ...
    omega_old = omega
    batch_B = random.sample(list(range(sarah.num)), size_B)
    v = sarah.difference_logistic_grad(omega, batch_B)
    v_old = v
    omega_h1 = omega_old - eta * v
    omega_h2 = prox_l1_norm(omega_h1, lam1, eta)
    omega = (1 - gamma) * omega_old + gamma * omega_h2
    ...
    for j in range(1, inner_loop+1):
        ...
        v = beta * v_old + beta * (sarah.difference_logistic_grad(omega, batch_b1) - sarah.difference_logistic_grad(omega_old, batch_b1)) \
            + (1 - beta) * sarah.difference_logistic_grad(omega, batch_b2)
        omega_old = omega
        v_old = v
        omega_h1 = omega_old - eta * v
        omega_h2 = prox_l1_norm(omega_h1, lam1, eta)
...
# plt.savefig('D:\Fig\p3_a9a_l.eps', dpi=600)
plt.show()
...
font1 = {'size': 7}
plt.legend(handles=[line1, line2, line3, line4, line5, line6], prop=font1)
# plt.savefig('D:\Fig\p3_a9a_G.eps', dpi=600)
plt.show()